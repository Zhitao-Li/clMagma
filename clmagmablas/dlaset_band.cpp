/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/zlaset_band.cpp, normal z -> d, Tue Jun 18 16:14:18 2019

       auto-converted from dlaset_band.cu

*/
#include "clmagma_runtime.h"
#include "common_magma.h"
#include "dlaset_band.h"
    ...
    magma_queue_t queue)
{
    ...
    if (uplo == MagmaUpper) {
        ...
        grid[0] *= threads[0];
        kernel = g_runtime.get_kernel( "dlaset_band_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        grid[0] *= threads[0];
        kernel = g_runtime.get_kernel( "dlaset_band_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(m        ), &m         );
