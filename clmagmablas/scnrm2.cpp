/*
    -- clMAGMA (version 1.0) --
       ...
       @generated from clmagmablas/dznrm2.cpp, normal z -> c, Tue Jun 18 16:14:18 2019
...
magmablas_scnrm2_adjust(int k, magmaFloat_ptr xnorm, size_t xnorm_offset, magmaFloatComplex_ptr c, size_t c_offset, magma_queue_t queue)
{
    ...
    kernel = g_runtime.get_kernel( "magmablas_scnrm2_adjust_kernel" );
    if (!kernel)
    {
                 ...
                 magmaFloat_ptr dxnorm, size_t dxnorm_offset, magma_queue_t queue) 
{
    ...
    kernel = g_runtime.get_kernel( "magmablas_scnrm2_kernel" );
    if (!kernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return MAGMA_ERR_UNKNOWN;
