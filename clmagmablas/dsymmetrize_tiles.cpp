/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/zsymmetrize_tiles.cpp, normal z -> d, Tue Jun 18 16:14:18 2019
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaUpper ) {
        kernel = g_runtime.get_kernel( "dsymmetrize_tiles_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        kernel = g_runtime.get_kernel( "dsymmetrize_tiles_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(m        ), &m         );
