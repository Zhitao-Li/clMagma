/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/zlange.cpp, normal z -> d, Tue Jun 18 16:14:18 2019

       auto-converted from dlange.cu
       @author Mark Gates
*/
    ...
    magma_queue_t queue )
{
    ...
    if ( norm == MagmaInfNorm ) {
        ...
        grid[0] *= threads[0];
        kernel = g_runtime.get_kernel( "dlange_inf_kernel" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        grid[0] = magma_ceildiv( m, NB_X );
        grid[0] *= threads[0];
        kernel = g_runtime.get_kernel( "dlange_max_kernel" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(m           ), &m            );
        ...
        grid[0] = n;
        grid[0] *= threads[0];
        kernel = g_runtime.get_kernel( "dlange_one_kernel" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(m           ), &m            );
