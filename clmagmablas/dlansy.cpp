/*
    -- MAGMA (version 1.1) --
       ...
       @author Mark Gates
       @generated from clmagmablas/zlanhe.cpp, normal z -> d, Tue Jun 18 16:14:18 2019
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "dlansy_inf_kernel_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        kernel = g_runtime.get_kernel( "dlansy_inf_kernel_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(n           ), &n            );
