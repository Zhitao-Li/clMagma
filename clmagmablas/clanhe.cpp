/*
    -- MAGMA (version 1.1) --
       ...
       @author Mark Gates
       @generated from clmagmablas/zlanhe.cpp, normal z -> c, Tue Jun 18 16:14:18 2019
...
clanhe_inf(
    ...
    magmaFloatComplex_const_ptr A, size_t A_offset, magma_int_t lda,
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "clanhe_inf_kernel_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        kernel = g_runtime.get_kernel( "clanhe_inf_kernel_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(n           ), &n            );
