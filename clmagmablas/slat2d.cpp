/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/clat2z.cpp, mixed zc -> ds, Tue Jun 18 16:14:18 2019
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    if (uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "slat2d_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        kernel = g_runtime.get_kernel( "slat2d_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(n        ), &n         );
