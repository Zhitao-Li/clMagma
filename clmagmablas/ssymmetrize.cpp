/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/zsymmetrize.cpp, normal z -> s, Tue Jun 18 16:14:18 2019
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaUpper ) {
        kernel = g_runtime.get_kernel( "ssymmetrize_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        kernel = g_runtime.get_kernel( "ssymmetrize_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(m        ), &m         );
