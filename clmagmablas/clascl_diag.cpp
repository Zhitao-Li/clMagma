/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/zlascl_diag.cpp, normal z -> c, Tue Jun 18 16:14:18 2019
...
magmablas_clascl_diag(
    ...
    magmaFloatComplex_const_ptr dD, size_t dD_offset, magma_int_t lddd,
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    if (type == MagmaLower) {
        kernel = g_runtime.get_kernel( "clascl_diag_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        kernel = g_runtime.get_kernel( "clascl_diag_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(m        ), &m         );
