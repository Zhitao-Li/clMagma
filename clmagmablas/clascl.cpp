/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/zlascl.cpp, normal z -> c, Tue Jun 18 16:14:18 2019
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    while( ! done ) {
        ...
        if (type == MagmaLower) {
            kernel = g_runtime.get_kernel( "clascl_lower" );
            if ( kernel != NULL ) {
                err = 0;
                arg = 0;
            ...
            kernel = g_runtime.get_kernel( "clascl_upper" );
            if ( kernel != NULL ) {
                err = 0;
                arg = 0;
                err |= clSetKernelArg( kernel, arg++, sizeof(m        ), &m         );
            ...
            kernel = g_runtime.get_kernel( "clascl_full" );
            if ( kernel != NULL ) {
                err = 0;
                arg = 0;
                err |= clSetKernelArg( kernel, arg++, sizeof(m        ), &m         );
