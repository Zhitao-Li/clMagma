/*
    -- clMAGMA (version 1.0) --
       ...
       @generated from clmagmablas/zgemm_reduce.cpp, normal z -> s, Tue Jun 18 16:14:19 2019
...
magmablas_sgemm_reduce(
    ...
    float alpha,
    magmaFloat_ptr d_A, size_t d_A_offset, magma_int_t lda,
    magmaFloat_ptr d_B, size_t d_B_offset, magma_int_t ldb,
    float beta,
    magmaFloat_ptr d_C, size_t d_C_offset, magma_int_t ldc,
    magma_queue_t queue)
{
    if (m%BLK_M != 0 || n%BLK_N != 0) {
        ...
        kernel = g_runtime.get_kernel( "magmablas_sgemm_reduce_kernel" );
        if (!kernel)
        {
        ...
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(float), (void*)&alpha );
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(magmaFloat_ptr), (void*)&d_A     );
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(int), (void*)&d_A_offset   );
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(int), (void*)&lda );
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(magmaFloat_ptr), (void*)&d_B     );
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(int), (void*)&d_B_offset   );
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(int), (void*)&ldb );
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(float), (void*)&beta     );
        ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(magmaFloat_ptr), (void*)&d_C     );
