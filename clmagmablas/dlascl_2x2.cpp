/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/zlascl_2x2.cpp, normal z -> d, Tue Jun 18 16:14:18 2019
...
magmablas_dlascl_2x2(
    ...
    magmaDouble_const_ptr dW, size_t dW_offset, magma_int_t lddw,
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    if (type == MagmaLower) {
        kernel = g_runtime.get_kernel( "dlascl_2x2_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        kernel = g_runtime.get_kernel( "dlascl_2x2_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(m        ), &m         );
