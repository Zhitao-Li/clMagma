/*
    -- clMAGMA (version 1.0) --
       ...
       @generated from clmagmablas/zlarfbx.cpp, normal z -> c, Tue Jun 18 16:14:19 2019
                  ...
                  magma_queue_t queue)
{
    ...
    kernel = g_runtime.get_kernel( "magma_cgemv_kernel1" );   // in clarfbx.cl
    if (!kernel)
    {
    ...
    ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(magmaFloatComplex_ptr), (void*)&c );
    ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(int), (void*)&c_offset    );
    ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(magmaFloatComplex_ptr), (void*)&dwork );
    ...
    kernel = g_runtime.get_kernel( "magma_ctrmv_tkernel" );   // in clarfx.cl
    if (!kernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return MAGMA_ERR_UNKNOWN;
    ...
    ciErrNum |= clSetKernelArg( kernel, nn++, sizeof(magmaFloatComplex_ptr), (void*)&dwork2 );
    ...
    kernel = g_runtime.get_kernel( "magma_cgemv_kernel2" );   // in clarfbx.cl
    if (!kernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return MAGMA_ERR_UNKNOWN;
