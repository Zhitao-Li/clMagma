/*
    -- MAGMA (version 1.1) --
       ...
       @generated from clmagmablas/zlacpy.cpp, normal z -> s, Tue Jun 18 16:14:18 2019
    ...
    if ( uplo == MagmaLower ) {
        for( unsigned int i=0; i < super_grid[0]; ++i ) {
            ...
            for( unsigned int j=0; j < super_grid[1] && j <= i; ++j ) {  // from left to diagonal
                ...
                if ( i == j ) {  // diagonal super block
                    kernel = g_runtime.get_kernel( "slacpy_lower_kernel" );
                    if ( kernel != NULL ) {
                        err = 0;
                        arg = 0;
                    ...
                    kernel = g_runtime.get_kernel( "slacpy_full_kernel" );
                    if ( kernel != NULL ) {
                        err = 0;
                        arg = 0;
                        dA_offset_ij = dA_offset + i*super_NB + j*super_NB*ldda;
        ...
        for( unsigned int i=0; i < super_grid[0]; ++i ) {
            ...
            for( unsigned int j=i; j < super_grid[1]; ++j ) {  // from diagonal to right
                ...
                grid[1] *= threads[1];
                if ( i == j ) {  // diagonal super block
                    kernel = g_runtime.get_kernel( "slacpy_upper_kernel" );
                    if ( kernel != NULL ) {
                        err = 0;
                        arg = 0;
                        dA_offset_ij = dA_offset + i*super_NB + j*super_NB*ldda;
