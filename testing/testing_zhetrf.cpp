    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (nopiv) {
                ...
                magma_zhetrf_nopiv( opts.uplo, N, h_A, lda, opts.queues2, &info);
                gpu_time = magma_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                gpu_time = magma_wtime();
                magma_zhetrf( opts.uplo, N, h_A, lda, ipiv, opts.queue, &info);
                gpu_time = magma_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                ...
                gpu_time = magma_wtime();
                magma_zhetrf_nopiv_gpu( opts.uplo, N, d_A, 0, ldda, opts.queues2, &info);
                gpu_time = magma_wtime() - gpu_time;
