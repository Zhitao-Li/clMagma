    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      for( int iuplo = 0; iuplo < 3; ++iuplo ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_int_t c_gpu = magma_znan_inf_gpu( uplo[iuplo], M, N, dA, 0, ldda, &c_gpu_nan, &c_gpu_inf, opts.queue );
            
            magma_int_t c_cpu2 = magma_znan_inf    ( uplo[iuplo], M, N, hA,    lda,  NULL, NULL );
            magma_int_t c_gpu2 = magma_znan_inf_gpu( uplo[iuplo], M, N, dA, 0, ldda, NULL, NULL, opts.queue );
