...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_zsetmatrix( m, n, hA, lda, dA, 0, ldda, opts.queue );
            magma_zgeqrf_gpu( m, n, dA, 0, ldda, tau, dT, 0, opts.queue, &info );
            if (info != 0)
                printf("magma_zgeqrf_gpu returned error %d: %s.\n",
                       (int) info, magma_strerror( info ));
            magma_zgetmatrix( m, n, dA, 0, ldda, hA, lda, opts.queue );
            lapackf77_zlacpy( MagmaFullStr, &m, &n, hA, &lda, hR, &lda );
            
            gpu_time = magma_wtime();
            if (opts.version == 1)
                magma_zungqr( m, n, k, hR, lda, tau, dT, 0, nb, opts.queue, &info );
            else
                magma_zungqr2(m, n, k, hR, lda, tau, opts.queue, &info );
