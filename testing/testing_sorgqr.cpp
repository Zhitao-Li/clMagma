/*
    -- MAGMA (version 1.1) --
       ...
       @generated from testing/testing_zungqr.cpp, normal z -> s, Tue Jun 18 16:14:26 2019
...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_ssetmatrix( m, n, hA, lda, dA, 0, ldda, opts.queue );
            magma_sgeqrf_gpu( m, n, dA, 0, ldda, tau, dT, 0, opts.queue, &info );
            if (info != 0)
                printf("magma_sgeqrf_gpu returned error %d: %s.\n",
                       (int) info, magma_strerror( info ));
            magma_sgetmatrix( m, n, dA, 0, ldda, hA, lda, opts.queue );
            lapackf77_slacpy( MagmaFullStr, &m, &n, hA, &lda, hR, &lda );
            
            gpu_time = magma_wtime();
            if (opts.version == 1)
                magma_sorgqr( m, n, k, hR, lda, tau, dT, 0, nb, opts.queue, &info );
            else
                magma_sorgqr2(m, n, k, hR, lda, tau, opts.queue, &info );
