/*
    -- clMAGMA (version 1.1) --
       ...
       @generated from testing/testing_zhetrf.cpp, normal z -> c, Tue Jun 18 16:14:25 2019
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (nopiv) {
                ...
                magma_chetrf_nopiv( opts.uplo, N, h_A, lda, opts.queues2, &info);
                gpu_time = magma_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                gpu_time = magma_wtime();
                magma_chetrf( opts.uplo, N, h_A, lda, ipiv, opts.queue, &info);
                gpu_time = magma_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                ...
                gpu_time = magma_wtime();
                magma_chetrf_nopiv_gpu( opts.uplo, N, d_A, 0, ldda, opts.queues2, &info);
                gpu_time = magma_wtime() - gpu_time;
