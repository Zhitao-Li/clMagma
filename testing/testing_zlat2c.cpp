    ...
    for( int iuplo = 0; iuplo < 2; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_zsetmatrix( n, n, A,  lda, dA,  0, ldda, opts.queue );
            magma_csetmatrix( n, n, SA, lda, dSA, 0, ldda, opts.queue );
            ...
            magmablas_zlat2c( uplo[iuplo], n, dA, 0, ldda, dSA, 0, ldda, opts.queue, &info );
            ...
            magma_cgetmatrix( n, n, dSA, 0, ldda, SR, lda, opts.queue );
            
            if ( opts.verbose ) {
                printf( "A=  " );  magma_zprint( n, n, A,  lda );
                printf( "SA= " );  magma_cprint( n, n, SA, lda );
                printf( "dA= " );  magma_zprint_gpu( n, n, dA,  0, ldda, opts.queue );
                printf( "dSA=" );  magma_cprint_gpu( n, n, dSA, 0, ldda, opts.queue );
            }
            ...
            gpu_time = magma_sync_wtime( opts.queue );
            magmablas_clat2z( uplo[iuplo], n, dSA, 0, ldda, dA, 0, ldda, opts.queue, &info );
            gpu_time = magma_sync_wtime( opts.queue ) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
            ...
            magma_zgetmatrix( n, n, dA, 0, ldda, R, lda, opts.queue );
