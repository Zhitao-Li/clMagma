/*
    -- MAGMA (version 1.1) --
       ...
       @generated from testing/testing_ztrsm.cpp, normal z -> c, Tue Jun 18 16:14:24 2019
...
int main( int argc, char** argv)
{
    ...
    // clBLAS 2.4 has errors in trsm causing aborts, so skip problematic cases
    bool skip_clblas = false;
    if (    (opts.side == MagmaLeft  && opts.uplo == MagmaLower && opts.transA == MagmaConjTrans) 
         || (opts.side == MagmaLeft  && opts.uplo == MagmaUpper && opts.transA == MagmaNoTrans  ) 
         || (opts.side == MagmaRight && opts.uplo == MagmaLower && opts.transA == MagmaNoTrans  ) 
         || (opts.side == MagmaRight && opts.uplo == MagmaUpper && opts.transA == MagmaConjTrans))
    {
        skip_clblas = true;
        fprintf( stderr, "\nWARNING: skipping clblasCtrsm( %s, %s, %s, %s ) due to errors.\n\n",
    ...
    printf("%% side = %s, uplo = %s, transA = %s, diag = %s \n",
           lapack_side_const(opts.side), lapack_uplo_const(opts.uplo),
           lapack_trans_const(opts.transA), lapack_diag_const(opts.diag) );
    printf("%%   M     N  MAGMA Gflop/s (ms)  clBLAS Gflop/s (ms)   CPU Gflop/s (ms)      MAGMA     clBLAS   LAPACK error\n");
    printf("%%===========================================================================================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_csetmatrix( Ak, Ak, h_A, lda, d_A, 0, ldda, opts.queue );
            ...
            bool cublas_okay = (cublas_error < tol);
            status += ! okay;
            cublas_status += ! cublas_okay;
            if ( opts.lapack ) {
                // check lapack
                ...
                printf("%5d %5d   %7.2f (%7.2f)   %7.2f (%7.2f)   %7.2f (%7.2f)   %8.2e   %8.2e   %8.2e   %s %s\n",
                        (int) M, (int) N,
                ...
                printf("%5d %5d   %7.2f (%7.2f)   %7.2f (%7.2f)     ---   (  ---  )   %8.2e   %8.2e     ---      %s %s\n",
                        (int) M, (int) N,
                        magma_perf,  1000.*magma_time,
                        cublas_perf, 1000.*cublas_time,
