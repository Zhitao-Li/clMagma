    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_zsetmatrix( M,   N,  C, ldc, dC, 0, ldc, opts.queue );
            magma_zsetmatrix( ldv, nv, V, ldv, dV, 0, ldv, opts.queue );
            magma_zsetmatrix( K,   K,  T, ldt, dT, 0, ldt, opts.queue );
            ...
            magma_zlarfb_gpu( side[iside], trans[itran], direct[idir], storev[istor],
                              ...
                              dC, 0, ldc,
                              dW, 0, ldw, opts.queue );
            magma_zgetmatrix( M, N, dC, 0, ldc, R, ldc, opts.queue );
