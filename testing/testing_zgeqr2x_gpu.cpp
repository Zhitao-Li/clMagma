...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magmablas_zlaset( MagmaFull, N, N, c_zero, c_zero, ddA,  0, N, opts.queue );
            magmablas_zlaset( MagmaFull, N, N, c_zero, c_zero, d_T,  0, N, opts.queue );
            magmablas_zlaset( MagmaFull, N, N, c_zero, c_zero, ddA2, 0, N, opts.queue );
            magmablas_zlaset( MagmaFull, N, N, c_zero, c_zero, d_T2, 0, N, opts.queue );
            ...
            magma_zsetmatrix( M, N, h_R, lda, d_A2, 0, ldda, opts.queue );
            ...
            //else if (opts.version == 3)
                ...
                magma_zgeqr2x3_gpu( M, N, d_A, 0, ldda, dtau, 0, d_T, 0, ddA, 0, dwork, 0, opts.queue, &info );
            ...
            //    //magma_zgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info, queues[1]);
            //    //magma_zgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, queues[0]);
            ...
            if (info != 0) {
                ...
                if ( opts.check ) {
                    ...
                    magma_zgetmatrix( N, N, ddA, 0, N,    h_T, N, opts.queue );
                    magma_zgetmatrix( min_mn, 1, dtau, 0, min_mn,   tau, min_mn, opts.queue );
                    ...
                    magma_zgetmatrix( N, N, d_T, 0, N, h_T, N, opts.queue );
