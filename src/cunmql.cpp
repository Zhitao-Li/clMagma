/*
    -- clMAGMA (version 0.4) --
       ...
       @generated from src/zunmql.cpp, normal z -> c, Tue Jun 18 16:14:17 2019
...
/*  -- MAGMA (version 0.4) --
    ...
    *info  = 0;
    ...
    if ( nb >= k )
      {
        /* Use CPU code */
        lapackf77_cunmql( lapack_side_const(side), lapack_trans_const(trans), &m, &n, &k, &a[a_offset], &lda, &tau[1],
    ...
    else
      {
        ...
        for (i__ = i1; i3 < 0 ? i__ >= i2 : i__ <= i2; i__ += i3) {
          ...
          /* 1) Put 0s in the lower triangular part of A;
             ...
             3) restore A                                      */
          magma_cpanel_to_q( MagmaLower, ib, &a[i__ + i__ * lda], lda, t+ib*ib );
          magma_csetmatrix( i__4, ib, &a[1 + i__ * lda], lda, dwork, 0, i__4, queue );
          magma_cq_to_panel( MagmaLower, ib, &a[i__ + i__ * lda], lda, t+ib*ib );
