/*
    -- clMAGMA (version 0.1) --
       ...
       @author Stan Tomov
       @generated from src/zgeqrf2_gpu.cpp, normal z -> d, Tue Jun 18 16:14:16 2019
...
/*  -- clMAGMA (version 0.1) --
    ...
    */
    ...
    work = (double*)clEnqueueMapBuffer(queues[0], buffer, CL_TRUE, 
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k-nx; i += nb) {
            ...
            magma_dgetmatrix_async(rows, ib, dA(i, i), ldda, work(i), ldwork, queues[0], NULL);
            ...
            magma_dsetmatrix_async(rows, ib, work(i), ldwork, dA(i,i), ldda, queues[0], NULL);

            /* download the T matrix */
            magma_queue_sync( queues[1] );
            magma_dsetmatrix_async( ib, ib, hwork, ib, dwork, 0, lddwork, queues[0], NULL);
            magma_queue_sync( queues[0] );
            ...
            if (i + ib < n) {
                if (i+nb < k-nx) {
                    ...
                    magma_dlarfb_gpu( MagmaLeft, MagmaConjTrans, MagmaForward, MagmaColumnwise,
                                      ...
                                      dA(i, i+ib), ldda, dwork,ib, lddwork, queues[1]);
                    magma_dq_to_panel( MagmaUpper, ib, work(i), ldwork, hwork+ib*ib );
                    magma_dsetmatrix_async(ib, ib, work(i), ldwork, dA(i,i), ldda, queues[1], NULL);
                }
                old_i  = i;
                old_ib = ib;
            }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_dgetmatrix_async(rows, ib, dA(i, i), ldda, work, rows, queues[1], NULL);
        magma_queue_sync( queues[1] );
        
        lhwork = lwork - rows*ib;
        lapackf77_dgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_dsetmatrix_async(rows, ib, work, rows, dA(i, i), ldda, queues[1], NULL);
    }
