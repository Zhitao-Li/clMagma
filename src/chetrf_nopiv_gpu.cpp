/*
    -- clMAGMA (version 1.1) --
       ...
       @generated from src/zhetrf_nopiv_gpu.cpp, normal z -> c, Tue Jun 18 16:14:17 2019
*/
    ...
    magma_queue_t queues[2], magma_int_t *info)
{
    ...
    *info = 0;
    ...
    // CPU workspace
    magmaFloatComplex *A;
    if (MAGMA_SUCCESS != magma_cmalloc_cpu( &A, nb*nb )) {
    ...
    if (upper) {
        ...
        for (j=0; j<n; j += nb) {
            ...
            magma_cgetmatrix_async(jb, jb, dA(j, j), ldda, A(j,j), nb, queues[0], NULL);
            trace_gpu_end( 0, 0 );
            ...
            magma_csetmatrix_async(jb, jb, A(j, j), nb, dA(j, j), ldda, queues[0], NULL);
            trace_gpu_end( 0, 0 );
