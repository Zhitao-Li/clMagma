/*
    -- clMAGMA (version 0.1) --
       ...
       @author Stan Tomov
       @generated from src/zgeqrf.cpp, normal z -> d, Tue Jun 18 16:14:17 2019
    ...
    *info = 0;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            magma_dsetmatrix_async( rows, ib, A(i,i), lda, dA(i,i), ldda, queues[0], NULL );

            /* download the T matrix */
            magma_queue_sync( queues[1] );
            magma_dsetmatrix_async( ib, ib, work, ib, dT, dT_offset, nb, queues[0], NULL );
            magma_queue_sync( queues[0] );
