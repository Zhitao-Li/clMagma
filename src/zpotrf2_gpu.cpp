    ...
    magma_int_t*   info)
{
    ...
    */
    ...
    work = (magmaDoubleComplex*)clEnqueueMapBuffer( queues[0], buffer, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, nb*nb*sizeof(magmaDoubleComplex), 0, NULL, NULL, NULL );

    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_zgetmatrix_async( n, n, dA(0,0), ldda, work, n, queues[1], NULL );
        magma_queue_sync( queues[1] );
        lapackf77_zpotrf( uplo_, &n, work, &n, info );
        magma_zsetmatrix_async( n, n, work, n, dA(0,0), ldda, queues[1], NULL );
    }
    else {
        // Use blocked code.
        if (upper) {
