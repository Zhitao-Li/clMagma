/*
    -- clMAGMA (version 0.1) --
       ...
       @generated from src/zgesv.cpp, normal z -> d, Tue Jun 18 16:14:16 2019
...
/*  -- clMAGMA (version 0.1) --
    ...
    *info = 0;
    ...
    magma_dsetmatrix( n, n, A, lda, dA, 0, ldda, queues[0] );
    magma_dgetrf2_gpu( n, n, dA, 0, ldda, ipiv, queues, info );
    if ( *info == MAGMA_ERR_DEVICE_ALLOC ) {
        magma_free( dA );
        magma_free( dB );
        goto CPU_INTERFACE;
    }
    magma_dgetmatrix( n, n, dA, 0, ldda, A, lda, queues[0] );
    if ( *info == 0 ) {
        magma_dsetmatrix( n, nrhs, B, ldb, dB, 0, lddb, queues[0] );
        magma_dgetrs_gpu( MagmaNoTrans, n, nrhs, dA, 0, ldda, ipiv, dB, 0, lddb, queues[0], info);
        magma_dgetmatrix( n, nrhs, dB, 0, lddb, B, ldb, queues[0]);
    }
    magma_free( dA );
    magma_free( dB );
    return *info;
...
CPU_INTERFACE:
    ...
    magma_dgetrf( n, n, A, lda, ipiv, queues, info );
    if ( *info == 0 ) {
