...
/*  -- clMAGMA (version 0.1) --
    ...
    *info = 0;
    ...
    magma_zsetmatrix( n, n, A, lda, dA, 0, ldda, queues[0] );
    magma_zgetrf2_gpu( n, n, dA, 0, ldda, ipiv, queues, info );
    if ( *info == MAGMA_ERR_DEVICE_ALLOC ) {
        magma_free( dA );
        magma_free( dB );
        goto CPU_INTERFACE;
    }
    magma_zgetmatrix( n, n, dA, 0, ldda, A, lda, queues[0] );
    if ( *info == 0 ) {
        magma_zsetmatrix( n, nrhs, B, ldb, dB, 0, lddb, queues[0] );
        magma_zgetrs_gpu( MagmaNoTrans, n, nrhs, dA, 0, ldda, ipiv, dB, 0, lddb, queues[0], info);
        magma_zgetmatrix( n, nrhs, dB, 0, lddb, B, ldb, queues[0]);
    }
    magma_free( dA );
    magma_free( dB );
    return *info;
...
CPU_INTERFACE:
    ...
    magma_zgetrf( n, n, A, lda, ipiv, queues, info );
    if ( *info == 0 ) {
