/*
    -- MAGMA (version 1.1) --
       ...
       @author Stan Tomov
       @author Mark Gates
       @generated from src/zpotrf2_gpu.cpp, normal z -> d, Tue Jun 18 16:14:15 2019
 */
    ...
    magma_int_t*   info)
{
    ...
    */
    ...
    work = (double*)clEnqueueMapBuffer( queues[0], buffer, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, nb*nb*sizeof(double), 0, NULL, NULL, NULL );

    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_dgetmatrix_async( n, n, dA(0,0), ldda, work, n, queues[1], NULL );
        magma_queue_sync( queues[1] );
        lapackf77_dpotrf( uplo_, &n, work, &n, info );
        magma_dsetmatrix_async( n, n, work, n, dA(0,0), ldda, queues[1], NULL );
    }
    else {
        // Use blocked code.
        if (upper) {
