/*
    -- clMAGMA (version 1.1) --
       ...
       @generated from src/zunmbr.cpp, normal z -> c, Tue Jun 18 16:14:18 2019
    ...
    magma_int_t *info)
{
    ...
    magma_int_t i1, i2, nb, mi, ni, nq, nw, iinfo, lwkopt;
    ...
    if (applyq) {
        ...
        if (nq >= k) {
            ...
            magma_cunmqr( side, trans,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, queue, &iinfo);
            #endif
        }
        else if (nq > 1) {
            /* Q was determined by a call to CGEBRD with nq < k */
            ...
            #if VERSION == 1
            magma_int_t nq_1 = nq - 1;
            lapackf77_cunmqr( lapack_side_const(side), lapack_trans_const(trans),
                              &mi, &ni, &nq_1, A(1,0), &lda, tau, C(i1,i2), &ldc, work, &lwork, &iinfo);
            #else
            magma_cunmqr( side, trans,
                          mi, ni, nq-1, A(1,0), lda, tau, C(i1,i2), ldc, work, lwork, queue, &iinfo);
            #endif
        }
    }
    else {
        ...
        if (nq > k) {
            ...
            #else
            magma_cunmlq( side, transt,
                          mi, ni, nq-1, A(0,1), lda, tau, C(i1,i2), ldc, work, lwork, queue, &iinfo);
            #endif
        }
    }
    work[0] = MAGMA_C_MAKE( lwkopt, 0 );
