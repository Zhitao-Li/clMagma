/*
    -- clMAGMA (version 0.1) --
       ...
       @author Stan Tomov
       @generated from src/zpotrf.cpp, normal z -> s, Tue Jun 18 16:14:16 2019
*/
    ...
    if (nb <= 1 || nb >= n) {
        ...
        if (uplo == MagmaUpper) {
            ...
            for (j=0; j<n; j += nb) {
                ...
                jb = min(nb, (n-j));
                magma_ssetmatrix_async( jb, (n-j), A(j, j), lda, dA(j, j), ldda, queues[1], NULL);
            ...
            for (j=0; j<n; j+=nb) {
                ...
                //  for non-positive-definiteness. Computing MIN
                jb = min(nb, (n-j));
                magma_ssetmatrix_async( (n-j), jb, A(j, j), lda, dA(j, j), ldda, queues[1], NULL);
