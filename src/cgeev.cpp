/*
    -- MAGMA (version 1.1) --
       ...
       @generated from src/zgeev.cpp, normal z -> c, Tue Jun 18 16:14:17 2019
       @author Stan Tomov
       @author Mark Gates
*/
 ...
 *
 * newer versions not yet in clMAGMA
    ...
    magma_int_t *info )
{
    ...
    #define dT(i,j)  dT,  ((i) + (j)*nb)  // cl_mem and offset
    ...
    *info = 0;
    ...
    if (wantvl) {
        ...
        #elif defined(VERSION3)
            // Version 3 - LAPACK consistent MAGMA HRD + T matrices stored
            magma_cunghr( n, ilo, ihi, VL, ldvl, &work[itau], dT(0,0), nb, queue, &ierr );
        #endif
        ...
        #elif defined(VERSION3)
            // Version 3 - LAPACK consistent MAGMA HRD + T matrices stored
            magma_cunghr( n, ilo, ihi, VR, ldvr, &work[itau], dT(0,0), nb, queue, &ierr );
        #endif
        time_sum += timer_stop( time_unghr );
        flop_sum += flops_stop( flop_unghr );
    ...
    if (wantvl || wantvr) {
        ...
        magma_ctrevc3( side, MagmaBacktransVec, select, n, A, lda, VL, ldvl,
                       VR, ldvr, n, &nout, &work[iwrk], liwrk, &rwork[irwork], queue, &ierr );
